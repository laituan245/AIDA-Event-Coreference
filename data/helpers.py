...
def load_aida_dataset(cs_filepath, ltf_dir, tokenizer):
    ...
    with open(cs_filepath, 'r', encoding='utf8') as f:
        for line in f:
            ...
            es = line.split('\t')
            event_id = es[0][1:]
            if not event_id in event2info:
                event2info[event_id] = {'arguments': []}
            if es[1] == 'mention.actual':
                ...
                # Append the argument to the events
                event2info[event_id]['arguments'].append({
                    'role': argument_role,
                    'entity_id': entity_id
                })
    ...
    for doc_id in aida_doc_ids:
        ...
        doc = Document(doc_id, [words], event_mentions, entity_mentions, 'AIDA')
